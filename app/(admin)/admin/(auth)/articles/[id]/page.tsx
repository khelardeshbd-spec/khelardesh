...
import { ArrowLeft, Check, Trash2, Camera, Plus, Sparkles, List, Quote, Link2, Tag, X } from 'lucide-react';
    ...
    fetch(`/api/admin/articles/${params.id}`)
      ...
      .then((data: any) => {
        ...
        if (found) {
          ...
          if (found.tags) {
            setTags(found.tags.split(',').map((s: string) => s.trim().replace(/^#/, '')).filter(Boolean));
          }
