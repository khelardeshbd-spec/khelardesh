...
export default function ProfileMenu({ user }: ProfileMenuProps) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (menuRef.current && !menuRef.current.contains(event.target as Node)) {
        setIsOpen(false);
        setTimeout(() => setActiveView('main'), 200); // Reset view after closing
      }
    }
  ...
  return (
    <div className="relative inline-block text-left" ref={menuRef}>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2.5 w-64 rounded-xl bg-white border border-gray-150 shadow-lg py-2 z-[999] origin-top-right transition-all animate-in fade-in slide-in-from-top-2 duration-150 overflow-hidden">
          
          <div 
            className="flex transition-transform duration-300 ease-in-out"
            style={{ 
              width: '200%', 
              transform: activeView === 'main' ? 'translateX(0)' : 'translateX(-50%)' 
            }}
          >
            {/* --- MAIN VIEW --- */}
              ...
              <div className="overflow-y-auto flex-1 pb-2 scrollbar-thin">
                {loadingNotifs ? (
                  ...
                  <div className="py-8 text-center text-gray-400 flex flex-col items-center">
                    <svg className="w-6 h-6 mb-1 opacity-50" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                    </svg>
                    <p className="text-[10px] font-medium">কোনো নোটিফিকেশন নেই</p>
                  </div>
                ) : (
